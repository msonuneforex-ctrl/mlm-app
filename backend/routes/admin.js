...
router.put('/deposits/:id/approve', (req, res) => {
  ...
  const netAmount = dep.net_amount || round2(dep.amount / 1.05);
  const feeAmount = dep.fee_amount || round2(dep.amount - netAmount);
  ...
  const tx = db.transaction(() => {
    db.prepare(`UPDATE deposits SET status='approved', net_amount = ?, fee_amount = ?, updated_at = datetime('now'), remarks = ? WHERE id = ?`)
      .run(netAmount, feeAmount, req.body.remarks || 'Approved by admin', dep.id);

    // Credit user's main wallet
    db.prepare('UPDATE users SET main_wallet = main_wallet + ? WHERE id = ?').run(netAmount, dep.user_id);
    db.prepare('UPDATE users SET wallet_balance = round(main_wallet + referral_wallet, 2) WHERE id = ?').run(dep.user_id);

    db.prepare(`INSERT INTO income_log (user_id, type, amount, note) VALUES (?, 'deposit_credit', ?, 'Deposit approved (platform fee deducted)')`)
      .run(dep.user_id, netAmount);

    // This deposit is principal, not income — it raises the user's earning
    // cap ceiling (2x net-invested capital) rather than counting against it.
    adjustInvestedCapital(dep.user_id, netAmount);
    ...
    if (admin) {
      ...
      db.prepare(`INSERT INTO income_log (user_id, type, amount, note) VALUES (?, 'platform_fee', ?, ?)`)
        .run(admin.id, feeAmount, `Platform fee from deposit #${dep.id}`);
    }

    // Direct Sponsor Referral Bonus
    if (depositUser && depositUser.sponsor_id && admin) {
      const bonus = round2(netAmount * REFERRAL_RATE);
      if (bonus > 0) {
        const sponsor = db.prepare('SELECT id, user_code, status FROM users WHERE id = ?').get(depositUser.sponsor_id);

        if (sponsor && sponsor.status === 'active') {
          // Sponsor is ACTIVE -> pay referral bonus out of admin wallet to sponsor's referral_wallet.
          ...
          db.prepare(`INSERT INTO income_log (user_id, type, amount, note) VALUES (?, 'referral_bonus', ?, ?)`)
            .run(sponsor.id, bonus, `Referral bonus from deposit #${dep.id} (from admin wallet)`);
        } else {
          // Sponsor is NOT active -> bonus is not paid out. No wallet change here:
          // the money was already credited to admin via the platform_fee entry
          // above, so "retaining" it just means skipping the payout — adding it
          // again would double-count 47.62 that the depositor never actually paid.
          db.prepare(`INSERT INTO income_log (user_id, type, amount, note) VALUES (?, 'referral_bonus_retained', ?, ?)`)
            .run(admin.id, bonus, `Referral bonus retained (sponsor ${sponsor ? sponsor.user_code : '#' + depositUser.sponsor_id} not ACTIVE) from deposit #${dep.id}`);
        }
      }
    }
  });
